...
void SysTick_Interrupt_Init(uint32_t clock_cycles, uint32_t priority)
{
    ...
    // Assign the number of clock cycles to be loaded to the LOAD register
    // Generate an interrupte every 1 ms
    ...
    SCB->SHP[11] = priority << 5;                                       // 0 highest priotity, 7 is lowest priotiy

    // Enable SysTick with interrupts and the core clock
    //SysTick->CTRL = 0x00000007;  // original
    SysTick->CTRL = 0x00000003;  // FIXED enable systick timer by writting 3
